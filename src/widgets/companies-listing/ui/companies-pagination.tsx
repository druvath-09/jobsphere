...
function CompaniesPagination({ currentPage, totalPages, totalResults, startIndex, endIndex, onPageChange }: CompaniesPaginationProps) {
  ...
  if (totalPages <= 1) {
    return (
      <div className="mt-8 flex flex-col gap-3 border-t border-border pt-5 text-sm text-text-secondary sm:flex-row sm:items-center sm:justify-between">
        <p>
          Showing {totalResults === 0 ? 0 : startIndex + 1}-{endIndex + 1} of {totalResults} companies
        </p>
        <p>Page 1 of 1</p>
      </div>
    );
  ...
  return (
    <div className="mt-8 flex flex-col gap-4 border-t border-border pt-5 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-sm text-text-secondary">
        Showing {startIndex + 1}-{endIndex + 1} of {totalResults} companies
      </p>
