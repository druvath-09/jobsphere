...
for (const company of companies) {
  let numJobs = 0;
  if (company.size <= 50) numJobs = Math.floor(Math.random() * 3) + 2; // 2-4
  else if (company.size <= 200) numJobs = Math.floor(Math.random() * 4) + 6; // 6-9
  else if (company.size <= 500) numJobs = Math.floor(Math.random() * 5) + 11; // 11-15
  else if (company.size <= 5000) numJobs = Math.floor(Math.random() * 5) + 16; // 16-20
  else numJobs = Math.floor(Math.random() * 5) + 22; // 22-26
...
const replacement = `const JOB_MOCKS: JobMock[] = ${JSON.stringify(jobs, null, 2)};`;
